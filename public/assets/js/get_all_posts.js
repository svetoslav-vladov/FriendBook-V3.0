...
function getAllPostsByLike(limit,offset) {
    ...
    req.open("GET", url_root + "/post/getAllPostsByLike&limit="+limit+"&offset="+offset);
    req.send();
}
    ...
    request.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            ...
            for(var post of posts) {
                ...
                $('#profile_news_feed').append(postCard);
            }
        }
    };
    request.open("GET", url_root + "/post/getOwnPosts&user_id="+user_id+"&limit="+limit+"&offset="+offset);
    request.send();
}
